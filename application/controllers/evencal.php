...
class Evencal extends CI_Controller {
	...
	public function show() {
			$data['h']=$this->evencal_model->select();
      $this->load->view('add_event', $data);
	}

	// for convert (int) month to (string) month
	...
	function add_event(){
		...
		function getData() {
			$data['groups'] = $this->evencal_model->getAllGroups();
			$this->load->view('add_event', $data);
		}
	...
	function _setting(){
		return array(
			'start_day' 		=> 'monday',
			'show_next_prev' 	=> true,
			'next_prev_url' 	=> site_url('index.php/evencal/index'),
			'month_type'   		=> 'long',
            'day_type'     		=> 'short',
